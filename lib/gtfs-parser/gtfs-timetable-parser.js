...
module.exports = (function(){
	...
	var calculateTripProgess = function(time, delta) {
                var dayOfWeek = day_to_text_day[time.getDay()];
		...
		for (var i in trips) {
                        var service_id = gtfs.getTripById(trips[i][0].trip_id).service_id;
                        var calendar = gtfs.getCalendarById(service_id);
                        /*console.log(calendar);*/
                        var is_running = calendar[dayOfWeek]==='1';
                        /*console.log("IS running: "+is_running);*/
                        //Trip is used only if it is running according to
                        //service dates
                        if (!is_running) {
                            //TODO: startdate and endDate in calendar aren't
                            //used
                            continue;
                        }
			...
			if ((startTime < time && time < endTime) || tripFits) {
				...
				// invert all progresses if direction bit is set
                                // All trips should move forward always
                                /*if (gtfs.getTripById(trips[i][0].trip_id).direction_id==1) {
                                    console.log("Inverting trip:"+trips[i][0].trip_id);
